        ...
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                if (ParkingArray[i][j] == "EMPTY") {
                    ...
                    string slotNumber = custom_string_concat(string(1, 'A' + i), custom_to_string(j + 1));
                    logVehicle(plateNum, slotNumber); // Log the vehicle
                    writeLogToFile(custom_string_concat("Parked: ", custom_string_concat(plateNum, custom_string_concat(" at slot ", slotNumber))));
                    writeCurrentParkedVehiclesToFile();
                    cout << "Vehicle with plate number " << plateNum << " is parked at slot " << slotNumber << ".\n";
                    return;
                }
            }
        }
...
void RetrieveVehicle(string plateNum) {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            if (ParkingArray[i][j] == plateNum) {
                ...
                removeLog(plateNum);
                writeLogToFile(custom_string_concat("Retrieved: ", custom_string_concat(plateNum, custom_string_concat(" from slot ", slotNumber))));
                writeCurrentParkedVehiclesToFile();
                cout << "Vehicle with plate number " << plateNum << " retrieved from slot " << slotNumber << ".\n";
    ...
    cout << "Vehicle with plate number " << plateNum << " not found in the parking lot.\n";
}
...
void SearchLicensePlate(string plateNum) {
    ...
    while (current != NULL) {
        if (current->plateNum == plateNum) {
            cout << "License plate " << plateNum << " is parked at slot " << current->slotNumber << ".\n";
            return;
        }
        current = current->next;
    }
...
void enqueue(string plateNum) {
    ...
    cout << "Vehicle with plate number " << plateNum << " added to the waiting queue.\n";
}
...
void dequeue() {
    ...
    delete temp;
    cout << "Vehicle with plate number " << plateNum << " removed from the waiting queue and parked.\n";
    ParkVehicle(plateNum);
}
    ...
    if (currentParkedFile.is_open()) {
        ...
        while (getline(currentParkedFile, line)) {
            ...
            if (atPos != string::npos) {
                ...
                int col = stoi(slotNumber.substr(1)) - 1; // Convert column number to index
...
string custom_string_concat(const string &str1, const string &str2) { //string concat replacement
    string result = str1;
    for (char c : str2) {
        result += c;
    }
